...
Firebase project — the backend that stores guardians, availability, and
raids, and syncs changes between everyone in real time. About 5 minutes.
GitHub repo with Pages enabled — hosts the static files. About 2 minutes.
Paste Firebase config into the code — one line of glue. About 30 seconds.
...
Click Add project. Give it any name (e.g. `pd2eb`). You can skip Google
Analytics — say no when prompted, it isn't needed for this.
Wait for the project to provision (~30 seconds).
1.2 Enable Anonymous Authentication
This is what lets your friends use the app without making accounts.
In the left sidebar, click Build → Authentication.
Click Get started.
On the Sign-in method tab, click Anonymous in the list.
Toggle Enable on, then Save.
1.3 Create the Realtime Database
This is where the shared data lives.
In the left sidebar, click Build → Realtime Database.
Click Create Database.
Pick a region (closest to your fireteam — `us-east1` is fine for ET).
When asked about security rules, choose Start in locked mode. We'll
replace these in the next step anyway.
Once the database exists, click the Rules tab.
Replace the contents with the contents of `database.rules.json` from this
repo:
```json
   {
     "rules": {
...
In the left sidebar, click the gear icon (top-left, next to "Project
Overview") → Project settings.
Scroll down to Your apps. Click the `</>` (web) icon to register a
web app.
Give it any nickname (e.g. `web`). Do not check "Set up Firebase
Hosting" — we're using GitHub Pages.
Click Register app.
Firebase shows you a config object that looks like this:
```js
   ...
   ```
Copy the whole object. You'll paste it into `app.js` shortly.
...
2.1 Create the repo
Go to https://github.com and sign in (or sign up — it's free).
Click New repository (top-right, green button or `+` menu).
Name it whatever you want — `fireteam-calendar`, `pd2eb`, etc.
Set it to Public. (GitHub Pages on private repos requires a paid plan.)
...
Drag in `index.html`, `style.css`, `app.js`, `database.rules.json`, and
...
In your repo, click `app.js` to open it.
Click the pencil icon (top-right of the file view) to edit.
Find the block near the top that says PASTE YOUR FIREBASE CONFIG HERE —
roughly lines 25 to 45.
Replace the `firebaseConfig` object's values with the ones from your
...
```js
   ...
   ```
No placeholder strings starting with `REPLACE\_` should remain.
Scroll down, click Commit changes.
...
Click Save.
Wait about a minute. Refresh the page. You'll see:
...
Part 3 — Test it
Open the URL.
Click Create guardian, pick a name and class.
Click some time slots — they should fill with gold tint and show a "1".
Open the URL in an incognito/private window and create a different
...
`\[20, 21, 22, 23, 0, 1, 2]`.
Different fireteam size: change `FIRETEAM\_SIZE` (currently 6). Set to 3
...
`REPLACE\_`. Re-check step 2.3.
"Could not connect" screen
Three common causes:
Anonymous auth isn't enabled in Firebase (step 1.2).
Your `databaseURL` is missing or wrong in the config.
The database rules are too restrictive — make sure they match
`database.rules.json`.
App loads but slots don't save
Open browser DevTools → Console. If you see `PERMISSION\_DENIED`, your
