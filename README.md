...
Designed to be self-hosted on GitHub Pages with Firebase Realtime Database as
the backend. Free tier on both is more than enough for a fireteam.
...
- **Different hours**: change the `HOURS` constant (currently 4 PM – 11 PM).
  E.g. for late-night raids, `[20, 21, 22, 23, 0, 1, 2]`.
- **Different fireteam size**: change `FIRETEAM_SIZE` (currently 6). Set to 3
  for dungeons, 12 for whatever you're cooking up.
- **Add raid names**: edit the `RAIDS` array.
- **Different colors**: edit the CSS variables at the top of `style.css`.
